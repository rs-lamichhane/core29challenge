...
import { MapPin, ArrowRight, Sparkles, Navigation, Map, Zap, ChevronDown, Radio } from 'lucide-react';
...
export default function JourneyInput({ userId, demoMode, onJourneyLogged }: Props) {
  ...
  useEffect(() => {
    ...
    // Detect user's city for smart routes
  ...
  return (
    <div className="mt-6 animate-fade-in-up">
      {/* Manual / Live Track toggle */}
      <div className="flex items-center justify-center gap-1 mb-4">
        <div className="bg-gray-100 dark:bg-gray-800 rounded-xl p-1 flex gap-1">
          <button
            onClick={() => setTrackingMode('manual')}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${trackingMode === 'manual'
              ? 'bg-white dark:bg-gray-700 text-brand-700 shadow-sm'
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold transition-all ${trackingMode === 'live'
              ? 'bg-white dark:bg-gray-700 text-brand-700 shadow-sm'
              : 'text-gray-500 hover:text-gray-700'
              }`}
          >
                ...
                <div className="flex gap-2 mb-3">
                  ...
                  <div className="relative flex-1">
                    ...
                    >
                      ...
                      {userCity ? `📍 Near ${userCity}` : t('btn.smartRoutes', language)}
