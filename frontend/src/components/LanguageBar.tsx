...
export default function LanguageBar() {
  ...
  return (
    <div className="w-full bg-white/60 dark:bg-gray-900/60 backdrop-blur-sm border-b border-gray-100 dark:border-gray-800 z-40 relative">
      <div className="max-w-4xl mx-auto px-4 py-1.5 flex items-center justify-end">
        <div ref={dropdownRef} className="relative">
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg text-xs font-medium text-gray-700 dark:text-gray-200 transition-all"
          >
            ...
            <div className="absolute top-full right-0 mt-1 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl z-50 overflow-hidden">
              {Object.entries(LANGUAGES).map(([code, { label, flag }]) => (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left text-xs transition-colors border-b border-gray-50 dark:border-gray-700 last:border-0 ${
                    language === code
